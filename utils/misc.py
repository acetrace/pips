...
def get_3d_embedding(xyz, C, cat_coords=True):
    ...
    y = xyz[:,:,1:2]
    z = xyz[:,:,2:3]
    div_term = (torch.arange(0, C, 2, device='mps', dtype=torch.float32) * (1000.0 / C)).reshape(1, 1, int(C/2))
    
    pe_x = torch.zeros(B, N, C, device='mps', dtype=torch.float32)
    pe_y = torch.zeros(B, N, C, device='mps', dtype=torch.float32)
    pe_z = torch.zeros(B, N, C, device='mps', dtype=torch.float32)
