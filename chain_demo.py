...
def run_model(model, rgbs, N, sw):
    rgbs = rgbs.to(torch.device('mps')).float() # B, S, C, H, W
    ...
    x = torch.ones((1, N), device=torch.device('mps')) * 450.0
    y = torch.ones((1, N), device=torch.device('mps')) * 100.0
    xy0 = torch.stack([x, y], dim=-1) # B, N, 2
    _, S, C, H, W = rgbs.shape

    trajs_e = torch.zeros((B, S, N, 2), dtype=torch.float32, device='mps')
    for n in range(N):
        # print('working on keypoint %d/%d' % (n+1, N))
        cur_frame = 0
        done = False
        traj_e = torch.zeros((B, S, 2), dtype=torch.float32, device='mps')
...
def main():
    ...
    model = Pips(stride=4).to(torch.device('mps'))
